...
def bulkInsert(record1,record2,record3):
    try:
        connection = psycopg2.connect(user="postgresuser",
                                      ...
                                      database="EDW-Database")
        cursor = connection.cursor()
        sql_insert_query1 = """ INSERT INTO ac (ac_power, dc_power, irradiance, time, date) VALUES (%s,%s,%s,%s,%s)"""
       # executemany() to insert multiple rows
        result = cursor.executemany(sql_insert_query1, record1)
        connection.commit()
        print(cursor.rowcount, "data inserted successfully into ac table")
        sql_insert_query2 = """ INSERT INTO weather (ghi, dhi, dni, windspeed, temp, time, date) VALUES (%s,%s,%s,%s,%s,%s,%s)"""
        sql_insert_query3 = """ INSERT INTO solar_position (app_zenith, zenith, app_elevation, elevation, azimuth, eq_time, time, date) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"""
        # executemany() to insert multiple rows
